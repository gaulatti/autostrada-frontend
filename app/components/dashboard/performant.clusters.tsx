...
import { ClusterPerformance, PerformanceComparisonCard, type ClusterPerformanceData, type PerformanceComparisonData } from './cards/performance';
  ...
  return (
    <div className='grid gap-6 md:grid-cols-3'>
      <Flex direction={'column'} align={'center'}>
        ...
        <span className='text-muted-foreground text-sm'>{t('dashboard.most-performant-clusters')}</span>
        <div className='space-y-4 w-full m-4'>
          {data.desktop.map((item) => (
            <ClusterPerformance {...(item as ClusterPerformanceData)} />
          ))}
        </div>
      </Flex>
      <Flex direction={'column'} align={'center'}>
      ...
      <Flex direction={'column'} align={'center'}>
        ...
        <div className='space-y-4 w-full m-4'>
          {data.differences.map((item) => (
            <PerformanceComparisonCard {...item as PerformanceComparisonData } />
          ))}
        </div>
      </Flex>
    </div>
